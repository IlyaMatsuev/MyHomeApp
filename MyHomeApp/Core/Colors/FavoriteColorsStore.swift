...
///
/// Favourites are a local convenience, so a persistence failure is logged rather than surfaced —
/// the list still works for the rest of the session.
@Observable
@MainActor
final class FavoriteColorsStore {
    private static let logger = Logger(subsystem: "MyHomeApp", category: "FavoriteColorsStore")

    private(set) var colors: [FavoriteColor] = []

    private let persistence: FavoriteColorsPersistence

    init(persistence: FavoriteColorsPersistence) {
        self.persistence = persistence
        do {
            colors = try persistence.load()
        } catch {
            Self.logger.error("Failed to load favorite colors: \(error.localizedDescription)")
        }
    }
    ...
    func add(_ hex: String) {
        ...
        colors.append(FavoriteColor(hex: normalized))
        persist()
    }

    func update(_ color: FavoriteColor, to hex: String) {
    ...
    func remove(_ color: FavoriteColor) {
    ...
    private func persist() {
        do {
            ...
            Self.logger.error("Failed to save favorite colors: \(error.localizedDescription)")
        }
    }
}
